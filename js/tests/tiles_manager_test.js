...
QUnit.test("TilesManager should update letters in tiles", function(assert){
    ...
    assert.equal(tilesManager.getLetterFromColumn(19,19), 'C', "Letter should have been set");
});
...
QUnit.test("TilesManager should prevent set letter at the edge when input is outside the boundary", function(assert) {
    ...
    assert.equal(tilesManager.getLetterFromColumn(oneColumnPastTheBoundary, 0), '', 'No letter should be set past the column boundary');

    var rightMostColumn = 19;
    assert.equal(tilesManager.getLetterFromColumn(rightMostColumn, 0), 'L', 'Letter should be set at the boundary');

    var oneRowPastTheBoundary = 20;
    tilesManager.setLetterToColumn('M', 0, oneRowPastTheBoundary);
    assert.equal(tilesManager.getLetterFromColumn(0, oneRowPastTheBoundary), '', 'No letter should be set past the column boundary');

    var lowestRow = 19;
    assert.equal(tilesManager.getLetterFromColumn(0, lowestRow), 'M', 'Letter should be set at the boundary');
    ...
    assert.equal(tilesManager.getLetterFromColumn(oneColumnLeftOfBoundary, oneRowAboveBoundary), '', 'No letter should be set past the column boundary');

    var upMostRow = 0;
    var leftMostColumn = 0;
    assert.equal(tilesManager.getLetterFromColumn(leftMostColumn, upMostRow), 'N', 'Letter should be set at the boundary');
